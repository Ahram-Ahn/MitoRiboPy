...
4. **Per-sample resolution stability** (v0.4+). When the TACO1-KO and control libraries use different kits (e.g. one TruSeq and one NEBNext UMI), per-sample adapter detection (the default since v0.7.1; `--kit-preset` was removed in that release) and per-sample dedup resolution still preserve the per-sample polyproline signal — the criterion above must hold for both samples regardless of kit chemistry.
