...
The 60/40 split is deliberate. The anchor reads provide a sharp peak at one offset value so the offset-selection routine has an unambiguous winner; the tile reads provide enough density across the CDS that the start-aligned metagene has an inspectable 3-nt pattern.
...
| Start-aligned metagene | Frame-1 mass == 0 across the whole 60-nt window | The fixture places P-sites on codon-aligned positions or at `stop - 2`; the off-by-one phase is the strongest negative control. |
...
| Output artefacts | `fourier_spectrum_combined.tsv`, `fourier_period3_score_combined.tsv`, and `periodicity.metadata.json` exist | Catches accidental output-path regressions in the current Fourier QC contract. |
...
- Periodicity-bundle statistics (bootstrap CI over genes, circular-shift permutation null, `spectral_ratio_3nt_local`, `snr_call` tiering, edge cases). Those have dedicated unit tests in [`tests/test_fourier_spectrum.py`](../../tests/test_fourier_spectrum.py), [`tests/test_fourier_stats.py`](../../tests/test_fourier_stats.py), and [`tests/test_fourier_edge_cases.py`](../../tests/test_fourier_edge_cases.py).

## Why this fixture is in the repo

The architectural review asked for a tiny known-answer integration test that lets a future reviewer (or CI in a year) prove that the numerical claims in the README's "What the numbers mean" section still hold. This fixture is the smallest dataset that exercises offset selection + metagene QC + TE + ΔTE in one pass on real (bundled) annotation, with hand-computable answers at every stage.
