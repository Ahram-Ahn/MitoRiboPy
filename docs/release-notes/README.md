...
- [v0.5.0](v0.5.0.md): `mitoribopy rnaseq` from-FASTQ mode (cutadapt + bowtie2 + pyDESeq2 inside the subcommand) with auto SE/PE / adapter / UMI detection and auto-pseudo-replicate fallback for n=1 conditions; new `[fastq]` optional-dependency extra; four new TE / DE / QC plots (MA, TE bar by condition, TE heatmap, sample PCA)
