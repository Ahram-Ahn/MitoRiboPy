...
- [v0.5.1](v0.5.1.md): `mitoribopy rnaseq` gains four WT-vs-X comparison plots (`de_volcano_mrna`, `de_volcano_rpf`, `te_compare_scatter`, `te_log2fc_bar`); pyDESeq2 now fits the Ribo-seq subset too and writes `rpf_de_table.tsv`; new `--base-sample` / `--compare-sample` CLI aliases (with matching YAML keys) for `--condition-a` / `--condition-b`, mirroring the rpf config's `base_sample` key
