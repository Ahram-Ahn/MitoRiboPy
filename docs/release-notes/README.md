...
- [v0.5.0](v0.5.0.md): per-sample UMI / kit overrides, unified per-site output layout, strain rename (`h.sapiens` / `s.cerevisiae`), `short` footprint class, per-sample resume markers, intermediate-file cleanup, publication-quality codon-correlation plot, PyPI install, diagram redesign
