...
### Added
- **Wakigawa-style Fourier amplitude spectrum** (`mitoribopy.analysis.fourier_spectrum`). For each `(sample, read_length, gene, region)` — where region is the 100-nt window upstream of (`orf`) or downstream of (`utr3`) the canonical stop codon — emits the full one-sided amplitude spectrum over period 2-10 nt, plus per-gene period-3 amplitude / score and a per-(sample, length, region) combined summary. Replaces the legacy single-scalar `fft_period3_power.tsv`. Plot writer renders the published two-panel overlay (ORF on top, 3' UTR on bottom) per `(sample, read_length)`. Wakigawa's overlap-upstream genes (MT-ATP8, MT-ND4L) are scored individually but excluded from the combined-data figure and combined-stats columns; their `*_overlap_upstream.png` and `n_genes_overlap_upstream` columns surface the diagnostic view separately.
...
### Removed (BREAKING)
- **`fft_period3_power.tsv` and the `--fft-period3` / `--periodicity-fft-period3` flags.** The single-scalar `power[k=1/3] / median(background)` computation collapsed too much information (no auditable spectrum, no 3' UTR negative control, whole-CDS window dominated by transcript-length noise). Replaced by the Fourier amplitude spectrum bundle above. Migration: scripts that read `fft_period3_power.tsv` should switch to `fourier_period3_score.tsv` (per-gene `period3_amplitude` and `period3_score = period3_amplitude / mean(reference periods 4-10 nt)`) or `fourier_period3_summary.tsv` (per-(sample, read_length, region) median / max scores, with overlap-upstream genes split out into separate columns). CLI flags renamed to `--fourier-spectrum` / `--periodicity-fourier-spectrum` (default ON), `--fourier-window-nt` (default 100), `--fourier-period-min` / `--fourier-period-max`. The legacy `calculate_fft_period3_ratio` and `build_fft_period3_table` helpers are deleted.
